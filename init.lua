...
require('lazy').setup({
  ...
  ---------------------------------------------------------------------------------------------------------------------
  {
    ...
    config = function()
      require('auto-save').setup {
        enabled = true, -- Enables autosave by default
        execution_message = {
          message = function()
            return ''
          end, -- Hides the autosave message
        },
        events = { 'InsertLeave', 'TextChanged' }, -- Saves on exiting insert mode or changing text
        conditions = {
          exists = true,
          filename_is_not = {}, -- Exclude specific files if needed
          filetype_is_not = {}, -- Exclude specific filetypes if needed
          modifiable = true,
        },
        write_all_buffers = false, -- Only saves the active buffer
        debounce_delay = 135, -- Delay (ms) between file changes and autosave
      }
    end,
  },
  ...
  -- require('lazy').setup({
  {
    ...
    config = function()
      ...
      dap.adapters.lldb = {
        type = 'executable',
        command = '/usr/bin/lldb-dap-18', -- Use lldb-dap-18 instead of lldb-vscode
        name = 'lldb',
      }
  ...
  },
  {
    ...
    config = function()
      ...
      -- Automatically open and close the UI during debugging
